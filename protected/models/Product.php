...
class Product extends \app\components\TActiveRecord
{
	...
	public function rules()
	{
		return [
			...
			[['description', 'category_id', 'price'], 'required'],
			[['category_id', 'price', 'state_id', 'type_id', 'created_by_id'], 'integer'],
			[['created_on', 'updated_on'], 'safe'],
			[['image_file'], 'string', 'max' => 255],
			[['created_by_id'], 'exist', 'skipOnError' => true, 'targetClass' => Category::className(), 'targetAttribute' => ['created_by_id' => 'id']],
	...
	{
		return $this->hasOne(Category::className(), ['id' => 'created_by_id']);
	}
	...
	public function asJson($with_relations = false)
	{
		...
		if ($with_relations) {
			...
			if (is_array($list)) {
				...
				$json['createdBy0'] 	= $relationData;
			} else {
				$json['createdBy0'] 	= $list;
			}
		}
		return $json;
	}
