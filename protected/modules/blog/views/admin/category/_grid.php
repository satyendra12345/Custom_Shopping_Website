...
$('#bulk_delete_category-grid').click(function(e) {
	e.preventDefault();
	 var keys = $('#blog-category-grid-view').yiiGridView('getSelectedRows');
	 ...
	 if ( keys != '' ) {
		...
		if( ok ) {
			$.ajax({
				url  : '<?php echo Url::toRoute(['admin/category/mass','action'=>'delete','model'=>get_class($searchModel)])?>', 
				type : "POST",
				data : {
					ids : keys,
				},
				success : function( response ) {
					if ( response.status == "OK" ) {
						 $.pjax.reload({container: '#blog-category-grid-view'});
					}
				}
		     });
		}
