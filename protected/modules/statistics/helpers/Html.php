...
class Html extends \yii\helpers\Html
{
     ...
     * @return string
     */
    public static function beginPanel($title, $size = 'col-md-12', $panelClass = [
        'panel',
        'panel-default'
    ], $headingClass = [
        'panel-heading'
     ...
     * @return mixed
     */
    public static function actionButtons($model, $template = [
        'update',
        'delete'
    ])
    {
    ...
    {
        static::initDefaultButton('view', 'eye', [
            'class' => 'btn btn-info'
        ]);
        ...
        if (Yii::$app->user->can("update{$className}", [
            'model' => $model
        ]) || Yii::$app->user->can("updateOwn{$className}", [
            'model' => $model
        ])) {
            static::initDefaultButton('update', 'pencil', [
                'class' => 'btn btn-primary'
            ]);
        }

        if (Yii::$app->user->can("approve{$className}", [
            'model' => $model
        ]) || Yii::$app->user->can("approveOwn{$className}", [
            'model' => $model
        ])) {
            static::initDefaultButton('approve', 'check', [
                'class' => 'btn btn-success'
            ]);
        }

        if (Yii::$app->user->can("delete{$className}", [
            'model' => $model
        ]) || Yii::$app->user->can("deleteOwn{$className}", [
            'model' => $model
        ])) {
