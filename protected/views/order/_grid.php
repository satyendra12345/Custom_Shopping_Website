...
<?php if (User::isAdmin()) echo Html::a('','#',['class'=>'multiple-delete glyphicon glyphicon-trash','id'=>"bulk_delete_order-grid"])?>
...
$('#bulk_delete_order-grid').click(function(e) {
	e.preventDefault();
	 ...
	 if ( keys != '' ) {
		...
		if( ok ) {
			$.ajax({
				url  : '<?php echo Url::toRoute(['order/mass','action'=>'delete','model'=>get_class($searchModel)])?>', 
				type : "POST",
				data : {
					ids : keys,
				},
